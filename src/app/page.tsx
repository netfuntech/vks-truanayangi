...
import { ArrowUpRight, AudioLines, Volume2, VolumeX, Sparkles, Utensils } from 'lucide-react';
...
function FoodImage({food,language}:{food:Food;language:Language}){
 if(food.src)return <div role="img" aria-label={foodName(food,language)} className="food-image menu-photo" style={{backgroundImage:`url(${basePath}${food.src})`}}/>;
...
export default function Home(){
 ...
 const [sound,setSound]=useState(true),[spinning,setSpinning]=useState(false),[result,setResult]=useState<Food|null>(null),[revealed,setRevealed]=useState(false);
 ...
 useEffect(()=>{const last=readCookie<{name?:unknown;price?:unknown}>('last-choice');if(last&&typeof last==='object'){const match=eligible.find(f=>f.name===last.name&&f.price===last.price);if(match)setResult(match)}},[eligible]);
 ...
 function open(){
  ...
  for(let id=last+1;id<=target+4;id++){
   ...
   const food=id===target?winner:pick(alternatives.length?alternatives:eligible);
 ...
 <div className="control-bar simple"><div className="open-wrap"><button className="open-button" disabled={spinning||!eligible.length} onClick={open}>{spinning?<AudioLines size={22}/>:<Sparkles size={21}/>} {spinning?t.opening:result?t.openAgain:t.open} <span>↗</span></button></div></div>
 <Dialog open={revealed} onOpenChange={setRevealed}><DialogContent className="winner-dialog" showCloseButton={false}>{result&&<><span className="winner-label">{t.newItem}</span><DialogTitle className="winner-title">{foodName(result,language)}</DialogTitle><DialogDescription className="winner-description">{t.referencePrice} · {priceLabel(result.price,language,true)} {t.perPerson}</DialogDescription><div className="winner-art" style={{'--rarity':colors[result.rarity]} as React.CSSProperties}><FoodImage food={result} language={language}/></div><LocationsList language={language}/><div className="winner-actions"><a className="grabfood-button" href={`https://food.grab.com/vn/vi/restaurants?${new URLSearchParams({search:result.name,'support-deeplink':'true',searchParameter:result.name})}`} target="_blank" rel="noreferrer" aria-label={language==='vi'?`Đặt ${result.name} qua GrabFood`:`Find ${foodName(result,language)} on GrabFood`}><span className="grabfood-label">{language==='vi'?'Đặt qua':'Order on'} <strong>GrabFood</strong></span><ArrowUpRight size={17} aria-hidden="true"/></a><button onClick={()=>setRevealed(false)}>{t.continue}</button></div></>}</DialogContent></Dialog>

 <section className="inventory"><div className="section-heading"><div><span className="eyebrow">{t.whatsInside}</span><div className="inventory-title-row"><h2>{t.items} <span>{eligible.length.toString().padStart(2,'0')}</span></h2></div></div><div className="rarity-legend">{t.tiers.map((tier,i)=><span key={tier}><i style={{background:colors[i]}}/>{tier}</span>)}</div></div><div className="inventory-grid">{inventoryCards}</div></section>
